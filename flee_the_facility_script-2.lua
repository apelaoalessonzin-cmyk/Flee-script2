...
local function IsComputerCompleted(computer)
    ...
    -- Conta partes NEON verdes e azuis
    for _, obj in pairs(computer:GetDescendants()) do
        if obj:IsA("BasePart") and obj.Material == Enum.Material.Neon then
            ...
            if c.G > 0.4 and c.G > c.R and c.G > c.B then
                greenNeons = greenNeons + 1
            end
            
            -- É azul?
            if c.B > 0.4 and c.B > c.R and c.B > c.G then
                blueNeons = blueNeons + 1
            end
        end
    end
    
    -- Se tem mais verde que azul E tem pelo menos 1 verde = COMPLETADO!
    -- (evita falso positivo de PCs sem neon nenhum)
    return greenNeons > 0 and greenNeons > blueNeons
end
