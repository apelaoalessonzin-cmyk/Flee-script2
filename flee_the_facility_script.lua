...
-- Torna notificações do Rayfield fantasmas (não bloqueia cliques)
spawn(function()
    while true do
        wait(0.5)
                ...
                for _, frame in pairs(gui:GetDescendants()) do
                    if frame:IsA("Frame") or frame:IsA("TextLabel") or frame:IsA("TextButton") then
                        if frame:IsA("TextLabel") or frame:IsA("TextButton") then
                            ...
                            if text:lower():find("discord") or text:lower():find("credit") or text:lower():find("server") then
                                -- Torna o elemento fantasma
                                frame.Active = false
                                frame.ZIndex = -1000
                                if frame.Parent then
                                    frame.Parent.Active = false
                                    frame.Parent.ZIndex = -1000
                                end
                            end
                        end
                    end
...
-- ESP COMPUTADORES
local computerStableStates = {} -- Rastreia estabilidade

local function IsComputerCompleted(computer)
    -- Procura o BoolValue "Value"
    local valueObj = nil
    for _, obj in pairs(computer:GetDescendants()) do
        if obj:IsA("BoolValue") and obj.Name == "Value" then
            valueObj = obj
            break
        end
    end
    ...
    -- Se não tem histórico, cria
    if not computerStableStates[computer] then
        computerStableStates[computer] = {
            lastValue = valueObj.Value,
            stableCount = 0,
            lastCheck = tick()
        }
        return false
    ...
    local state = computerStableStates[computer]
    local currentValue = valueObj.Value
    
    -- Se o valor mudou, reseta contador
    if state.lastValue ~= currentValue then
    ...
    -- Se está estável por mais de 1 segundo E está em false = COMPLETADO!
    if currentValue == false and (tick() - state.lastCheck) > 1 then
        state.stableCount = state.stableCount + 1
        if state.stableCount >= 2 then
            return true -- Confirmado como completado!
        end
    end
...
local function ApplyFullBright()
    ...
    if FullBrightEnabled then
        ...
        L.Brightness = 1.5
        L.ColorShift_Bottom = Color3.fromRGB(178, 178, 178)
        L.ColorShift_Top = Color3.fromRGB(178, 178, 178)
        ...
        L.ClockTime = 12
        -- NÃO reativa os efeitos para evitar embaçar
    end
end
