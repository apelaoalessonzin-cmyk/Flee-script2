...
task.wait(2)
spawn(function()
    local blockedWords = {"discord", "sirius", "enjoying", "library", "find it at", "notification", "settings"}
    while true do
        wait(0.5)
        pcall(function()
            for _, gui in pairs(game.CoreGui:GetChildren()) do
                -- Só mexe em GUIs do Rayfield, não do Delta
                if gui.Name:find("Rayfield") then
                    for _, obj in pairs(gui:GetDescendants()) do
                        pcall(function()
                            -- Remove notificações
                            if obj.Name == "Notifications" or obj.Name == "Notification" or obj.Name == "Settings" then
                                obj:Destroy()
                            end
                            -- Remove textos com palavras bloqueadas
                            if obj:IsA("TextLabel") or obj:IsA("TextButton") then
                                local text = (obj.Text or ""):lower()
                                for _, word in pairs(blockedWords) do
                                    if text:find(word) then
                                        local target = obj.Parent
                                        if target and target.Name ~= "Rayfield" then
                                            target:Destroy()
                                        end
                                        break
                                    end
