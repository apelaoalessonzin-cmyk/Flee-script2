...
local function AnalyzeComputer(computer)
    ...
    for _, obj in pairs(computer:GetDescendants()) do
        if obj:IsA("BasePart") and obj.Material == Enum.Material.Neon then
            ...
            local c = obj.Color
            local detail = string.format("Neon %s: RGB(%.0f,%.0f,%.0f) | BrickColor: %s", 
                obj.Name, c.R*255, c.G*255, c.B*255, tostring(obj.BrickColor))
    ...
    DebugLog("Completado: " .. tostring(info.greenNeons > 0 and info.greenNeons > info.blueNeons))
    DebugLog("================")
...
local function IsComputerCompleted(computer)
    ...
    local completed = greenNeons > 0 and greenNeons > blueNeons
    
    if DebugMode then
        DebugLog(string.format("Check PC: Verde=%d Azul=%d Completo=%s", greenNeons, blueNeons, tostring(completed)))
    end
