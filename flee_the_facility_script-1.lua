...
-- Remove e continua removendo qualquer menção a Discord/Sirius
spawn(function()
    while true do
        wait(1)
        for _, gui in pairs(game.CoreGui:GetChildren()) do
            if gui.Name == "Rayfield" then
                for _, obj in pairs(gui:GetDescendants()) do
                    pcall(function()
                        if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("ImageButton") or obj:IsA("ImageLabel") then
                            local text = (obj.Text or obj.Name or ""):lower()
                            if text:find("discord") or text:find("sirius") or 
                               text:find("server") or text:find("join") or
                               text:find("invite") then
                                obj:Destroy()
                            end
                        end
                        if obj:IsA("Frame") or obj:IsA("ScrollingFrame") then
                            local name = obj.Name:lower()
                            if name:find("discord") or name:find("credit") then
                                obj:Destroy()
                            end
                        end
                    end)
...
local function IsComputerCompleted(computer)
    ...
    for _, obj in pairs(computer:GetDescendants()) do
        if obj:IsA("BasePart") and obj.Material == Enum.Material.Neon then
            neonIsDarkGreen = (obj.BrickColor == BrickColor.new("Dark green"))
            break
        end
    end
    
    -- COMPLETADO = Value false E Neon Dark green JUNTOS!
    return hasValue and valueIsFalse and neonIsDarkGreen
end
    ...
    for _, obj in pairs(game.Workspace:GetDescendants()) do
        if obj.Name == "ComputerTable" and obj:IsA("Model") then
            if not ComputerESPObjects[obj] then
                -- Não cria ESP se já está completado
                if not IsComputerCompleted(obj) then
                    pcall(function()
                        ...
                        -- Loop que verifica se completou
                        spawn(function()
